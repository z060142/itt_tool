...
- **超長圖片處理** ⭐ NEW: 自動處理超長螢幕截圖（高度 > 3600px）
  - 智慧切割：將超長圖片切割成重疊的切片（18% 重疊防止題目被切斷）
  - OCR 識別：逐片進行文字識別，保留完整格式
  - 文字拼接：使用重疊區域去重，確保無遺漏
  - 題目提取：從完整文字中智慧提取題目和選項
  - 即時預覽：處理完成後可預覽提取結果再確認添加
- **智慧識別**: 使用OpenRouter的Qwen3 VL視覺模型，準確識別圖片中的文字內容
- **圖片管理**: 自動儲存題目相關圖片，支援點擊查看原圖
- **智慧縮放**: 上傳和儲存時自動等比縮小至短邊 1200px，節省流量和空間
...
- **批量答題**: 使用 AI 自動批量答題並生成注釋
- **批量注釋**: 為已有答案的題目批量生成注釋說明
- **圖片關鍵字偵測**: 自動偵測題目中的圖片關鍵字，智能決定是否發送圖片給 AI

### 其他功能

- **標點符號整理**: 自動統一中文標點符號，確保格式一致
...
適用於長螢幕截圖、多頁題目合併圖片等超長圖片（高度 > 3600px 或高寬比 > 3:1）：

1. 點擊「處理超長圖片」按鈕
2. 選擇一張超長圖片
3. 系統會自動執行以下流程：
   - **檢測尺寸**: 判斷圖片是否需要切割
   - **智慧切割**: 將圖片切成重疊的切片（防止題目被切斷）
   - **OCR 識別**: 逐片進行文字識別
   - **文字拼接**: 使用重疊區域去重合併
   - **題目提取**: 從完整文字中提取題目
4. 處理進度會即時顯示（切片、識別、拼接、提取）
5. 完成後可點擊「預覽題目」查看提取結果
6. 確認無誤後點擊「確認並添加」將題目加入題庫

**注意事項**：
- 處理時間較長（需多次 API 調用），請耐心等待
- 建議圖片中題目格式規範（1. 2. 3. 或 1) 2) 3)）
- 確保選項標記清晰（A. B. C. D. 或 (A) (B) (C) (D)）
- 處理前先預覽可避免錯誤添加
...
- `punctuation_mode`: 標點符號處理模式（`disabled`, `to_fullwidth`, `to_halfwidth`，預設 `disabled`）
- `auto_detect_image_keywords`: 自動偵測圖片關鍵字（`true`/`false`，預設 `false`）
...
### v1.6.0 (2025)
- **新增批量答題功能**: 使用 AI 自動批量答題
- **新增批量注釋功能**: 為已有答案的題目批量生成注釋
- **新增全局設定**: 統一管理系統全局參數
- **標點符號自動整理**:
  - 支援中文環境下自動轉換標點符號（半形 ↔ 全形）
  - 智能偵測中文環境，避免誤轉換其他語言
  - 在相似比對前執行，確保格式一致
- **圖片關鍵字自動偵測**:
  - 自動偵測題目中的圖片關鍵字（圖、圖片、image 等）
  - 偵測到時強制發送圖片給 AI
  - 批量答題/注釋時自動生效

### v1.5.0 (2025)
- **圖片上傳優化**: 上傳前在記憶體中自動縮放，節省 80-90% API 流量
- **儲存優化**: 使用處理過的圖片儲存，統一為 JPEG 格式
- **效能提升**: 減少重複處理，加快 API 回應速度
