...
3. **UI 層** (`main.py`)
   ...
   - **超長圖片處理**: `LongImageProgressDialog` 處理超長螢幕截圖

4. **圖片切割層** (`image_splitter.py`) - 處理超長圖片
   - `ImageSplitter`: 將超長圖片切割成重疊的切片
   - 自動檢測圖片尺寸（高度 > 3600px 或高寬比 > 3:1）
   - 切片重疊設計（18% 重疊）防止題目被切斷
   - 臨時檔案管理和自動清理

5. **文字處理層** (`text_processor.py`) - 處理 OCR 結果
   - `TextProcessor`: 合併 OCR 文字並提取題目
   - 使用 difflib 比對重疊區域並去重
   - 正則表達式識別題號和選項
   - 題目完整性驗證
...
**處理流程**：
```
超長圖片 → 檢測尺寸 → 切割圖片 → OCR識別 → 文字拼接 → 題目提取 → 預覽確認 → 批量添加
           (3600px)   (重疊18%)   (逐片)    (去重)     (正則)     (用戶)     (題庫)
```

**核心特性**：

1. **智慧切割**（ImageSplitter）
   - 觸發條件：高度 > 3600px 或 高寬比 > 3:1
   - 切片高度：1400px（可配置）
   - 重疊比例：18%（約 250px），確保題目不被切斷
   - 自動處理最後一片過小的情況

2. **重疊區域去重**（TextProcessor）
   - 取前一片末尾 N 行和當前片開頭 N 行比對
   - 使用 `difflib.SequenceMatcher` 計算相似度
   - 找到最佳匹配點並移除重複文字
   - 相似度閾值：0.6（可配置）

3. **題目提取**（TextProcessor）
   - 正則識別題號：`^\d+[\.\)、]`
   - 正則識別選項：`^[A-D][\.\)]`
   - 智慧處理多行題目和選項
   - 自動補齊缺失選項（標記為"無"）

4. **進度追蹤**（LongImageProgressDialog）
   - 即時進度條顯示（0-100%）
   - 詳細日誌輸出每個步驟
   - 非阻塞背景執行緒處理
   - 預覽功能讓用戶確認提取結果
...
**配置參數**（在 `process_long_image()` 方法中）：
```python
height_threshold=3600        # 高度閾值（px）
aspect_ratio_threshold=3.0   # 高寬比閾值
slice_height=1400            # 單片高度（px）
overlap_ratio=0.18           # 重疊比例（18%）
```
